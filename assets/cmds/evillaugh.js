...
param.message.member.getVoiceChannel().join(false,false).then(voice =>{
    var encoder = voice.voiceConnection.createExternalEncoder({type: "ffmpeg", source: "Pyro_laughevil0"+sliceArgs+".wav"});
    encoder.once("end", () => {console.log("Sucessfully played Pyro_laughevil0"+sliceArgs+".wav");
