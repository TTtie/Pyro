...
param.message.member.getVoiceChannel().join(false,false).then(voice =>{
    var encoder = voice.voiceConnection.createExternalEncoder({type: "ffmpeg", source: "Pyro_sf13_spell_generic0"+sliceArgs+".wav"});
    encoder.once("end", () => {console.log("Sucessfully played Pyro_sf13_spell_generic0"+sliceArgs+".wav");
